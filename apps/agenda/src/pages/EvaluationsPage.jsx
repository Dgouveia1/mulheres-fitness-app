...
function imcColor(v) {
  if (v == null) return 'text-content-subtle'
...
function EvolutionChart({ assessments }) {
  ...
  if (data.length < 2) {
    return <p className="text-sm text-content-muted text-center py-8">Precisa de pelo menos 2 avaliações para gerar o gráfico.</p>
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-3 text-xs">
        <span className={`emf-badge ${trendDown ? 'bg-emerald-500/10 text-emerald-600' : 'bg-amber-500/10 text-amber-600'}`}>
          <span className="material-icons text-sm">{trendDown ? 'trending_down' : 'trending_up'}</span>
          {Math.abs(last - first).toFixed(1)} kg
        </span>
        <span className="text-content-muted">no período</span>
      </div>
      <div className="relative flex items-end justify-between gap-1.5 sm:gap-2 h-44 px-1 border-b border-line">
        {data.map((d, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center justify-end h-full gap-1 group">
              <span className="text-[10px] font-bold text-content">{d.weight}</span>
              <div
                className="w-full max-w-[28px] bg-gradient-to-t from-primary to-primary-light rounded-t-lg transition-all group-hover:opacity-80"
                style={{ height: `${Math.max(8, pct)}%` }}
              />
              <span className="text-[9px] text-content-subtle">{date}</span>
            </div>
          )
        })}
      </div>
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-content">Avaliações Físicas</h1>
        <button
          onClick={() => { if (!selectedClient) { show('Selecione uma aluna primeiro.', 'warning'); return } setShowModal(true) }}
          className="emf-btn-primary rounded-full"
        >
      ...
      <div className="emf-card p-4">
        <label className="block text-xs font-semibold text-content-muted uppercase tracking-wider mb-2">Selecionar aluna</label>
        <select value={selectedClient} onChange={(e) => setSelectedClient(e.target.value)} className="emf-input">
      ...
      {selectedClient && (
        loadingAssessments ? (
          <div className="emf-card p-10 flex justify-center">
          ...
          <div className="text-center py-12 emf-card border-dashed">
            <div className="w-14 h-14 rounded-full bg-muted flex items-center justify-center mx-auto mb-2">
              <span className="material-icons text-content-subtle">assignment</span>
            </div>
            <p className="text-sm text-content-muted">Nenhuma avaliação registrada para {selectedClientName}.</p>
          </div>
        ) : (
          <>
            {/* Stats atuais */}
            <div className="grid grid-cols-3 gap-3">
              <div className="emf-card p-4 text-center">
                <div className="text-xs text-content-muted mb-1">IMC Atual</div>
                <div className={`text-2xl font-bold ${imcColor(latestImc)}`}>{latestImc ? latestImc.toFixed(1) : '--'}</div>
              </div>
              <div className="emf-card p-4 text-center">
                <div className="text-xs text-content-muted mb-1">Peso Atual</div>
                <div className="text-2xl font-bold text-content">{latest?.weight ?? '--'}<span className="text-sm font-semibold text-content-muted"> kg</span></div>
              </div>
              <div className="emf-card p-4 text-center">
                <div className="text-xs text-content-muted mb-1">Altura</div>
                <div className="text-2xl font-bold text-content">{latest?.height ?? '--'}<span className="text-sm font-semibold text-content-muted"> m</span></div>
              </div>
            </div>

            {/* Gráfico de evolução */}
            <div className="emf-card p-4">
              <h2 className="text-sm font-bold text-content mb-3">Evolução de Peso</h2>
              <EvolutionChart assessments={assessments} />
            </div>

            {/* Histórico */}
            <div className="emf-card overflow-hidden">
              <h2 className="text-sm font-bold text-content p-4 pb-2">Histórico de {selectedClientName}</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-xs text-content-muted border-b border-line">
                    ...
                    {assessments.map((a) => {
                      ...
                      return (
                        <tr key={a.id} className="border-b border-line/60 last:border-0">
                          <td className="px-4 py-2.5 text-content-muted">{new Date(a.created_at).toLocaleDateString('pt-BR')}</td>
                          <td className="px-2 py-2.5 font-semibold text-content">{a.weight} kg</td>
                          <td className="px-2 py-2.5 text-content-muted">{a.measurements?.waist ?? '—'}</td>
                          <td className="px-2 py-2.5 text-content-muted">{a.measurements?.hip ?? '—'}</td>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={`Nova Avaliação — ${selectedClientName}`} size="lg">
        <form onSubmit={handleSave} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-content-muted uppercase tracking-wider mb-1">Peso (kg)</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-content-muted uppercase tracking-wider mb-1">Altura (m)</label>
              ...
              <small className="text-[11px] text-content-subtle">Em branco mantém a anterior.</small>
            </div>
          </div>

          <div>
            <h4 className="text-xs font-bold text-content-muted uppercase tracking-wider mb-2 border-b border-line pb-1">Medidas (cm)</h4>
            <div className="grid grid-cols-3 gap-3">
              {MEASURES.map((m) => (
                <div key={m.key}>
                  <label className="block text-[11px] font-semibold text-content-muted mb-1">{m.label}</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-content-muted uppercase tracking-wider mb-1">Observações</label>
