...
import { getAdminStats, getAppointments } from '@/services/api'
...
export function AdminDashboardPage() {
  ...
  const QUICK_LINKS = [
    { to: '/admin/clientes', icon: 'group', label: 'Clientes', color: 'from-blue-500 to-blue-600' },
    { to: '/admin/treinos', icon: 'fitness_center', label: 'Treinos', color: 'from-primary to-primary-dark' },
    { to: '/admin/nutricao', icon: 'restaurant_menu', label: 'Nutrição', color: 'from-emerald-500 to-emerald-700' },
    { to: '/agenda', icon: 'calendar_month', label: 'Agenda', color: 'from-purple-500 to-purple-700' },
    { to: '/agenda/chat', icon: 'chat', label: 'Chat', color: 'from-amber-500 to-amber-600' },
    { to: '/admin/fitflix', icon: 'play_circle', label: 'FitFlix', color: 'from-red-500 to-red-600' },
  ]
        ...
        <div className="grid grid-cols-3 gap-3">
          {QUICK_LINKS.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className={`bg-gradient-to-br ${item.color} text-white rounded-2xl p-4 flex flex-col items-center gap-2 hover:opacity-90 active:scale-95 transition-all`}
            >
